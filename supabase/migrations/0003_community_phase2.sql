-- ────────────────────────────────────────────────────────────────────────────
-- Brand Engage Pro — Community Hub Phase 2a (polls, challenges, announcements)
...
-- ─── Poll votes (one vote per member per poll) ────────────────────────────────
...
-- ─── Challenge entries (member submissions to a challenge post) ───────────────
...
-- One entry per member per challenge — can be relaxed later if we want multiple.
create unique index if not exists community_challenge_entries_unique
  on public.community_challenge_entries (post_id, member_id);
...
begin
  if not exists (select 1 from points_ledger where source_ref = ref_id) then
    ...
    values (new.member_id, award, 'challenge', ref_id, 'Poll vote');
...
begin
  if not exists (select 1 from points_ledger where source_ref = ref_id) then
    insert into points_ledger (member_id, delta, source, source_ref, note)
    values (new.member_id, award, 'challenge', ref_id, 'Challenge submission');
...
-- Poll votes: authenticated read, each member can only vote as themselves.
...
-- Challenge entries: authenticated read, members can submit + withdraw their own.
