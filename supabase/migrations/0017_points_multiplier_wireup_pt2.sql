-- ────────────────────────────────────────────────────────────────────────────
-- Brand Engage Pro — Phase 5e #24: wire points_multiplier() into RSVP + member-action
--
...
--   award_member_action_points    dynamic (member_actions.point_value)
--
-- Design decisions
-- ────────────────
--
-- event_rsvps: straightforward. event_rsvps has event_id; brand_events has
-- brand_slug. One lookup to get the community, then the standard multiplier.
--
-- member_action_completions: member_actions.brand_slug is NULLABLE (global
-- actions don't belong to any single community). The product call we're
-- making here:
--
--     * Scoped actions (brand_slug IS NOT NULL) apply the multiplier based
--       on the member's membership in THAT community — same rule as posts.
--     * Global actions (brand_slug IS NULL) award the flat base points,
...
begin
  ...
  if not exists (select 1 from points_ledger where source_ref = ref_id) then
    ...
    values (
      new.member_id, award, 'event_rsvp', ref_id,
...
begin
  ...
  if not exists (select 1 from points_ledger where source_ref = v_ref) then
    insert into points_ledger (member_id, delta, source, source_ref, note)
    values (
      new.member_id, v_award, 'social_share', v_ref,
...
--    'award_event_rsvp_points','award_member_action_points'
--  );
--
...
--
-- 3. Sanity-check the multiplier for each (member, community) pair:
-- select community_id, subscription_tier, count(*),
--        public.points_multiplier(member_id, community_id) as sample_mult
--   from member_community_memberships
