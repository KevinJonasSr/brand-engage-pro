...
--
-- Creates `communities` (tenants), `member_community_memberships` (per-community
...
-- ─── 1. communities table ─────────────────────────────────────────────────
-- One row per tenant. `type` partitions brand communities from label-meta
...
create table if not exists public.communities (
  ...
  type            text not null check (type in ('brand','label_meta','brand')),
  tagline         text,
  bio             text,
...
insert into public.communities (slug, display_name, type, tagline, subdomain, active, sort_order) values
  ('raelynn',         'RaeLynn',                      'brand',     'Country, heart-first.',     'raelynn',      true,  1),
  ('danger-twins',    'Danger Twins · Amy Stroup',    'brand',     null,                        'dangertwins',  false, 2),
  ('dan-marshall',    'Dan Marshall',                 'brand',     null,                        'danmarshall',  false, 3),
  ('hunter-hawkins',  'Hunter Hawkins',               'brand',     null,                        'hunterhawkins',false, 4),
...
-- ─── 2. member_community_memberships ─────────────────────────────────────────
-- Per-community member state. A member who belongs to N communities has N rows
...
create table if not exists public.member_community_memberships (
...
create index if not exists member_community_memberships_member_idx
  on public.member_community_memberships (member_id);
create index if not exists member_community_memberships_community_idx
  on public.member_community_memberships (community_id, status);

-- Backfill: every existing member becomes a RaeLynn community member with
...
insert into public.member_community_memberships
  (member_id, community_id, joined_at, total_points, current_tier, referral_code, status)
select
  id,
  'raelynn',
...
-- community_posts: already has brand_slug. Copy to community_id.
alter table public.community_posts add column if not exists community_id text not null default 'raelynn';
update public.community_posts set community_id = brand_slug where brand_slug is not null and community_id = 'raelynn';
create index if not exists community_posts_community_idx
  on public.community_posts (community_id, created_at desc);

-- brand_events: same pattern. (Table stays named brand_events for now; a
-- later migration may rename to `events` once all callers are updated.)
alter table public.brand_events add column if not exists community_id text not null default 'raelynn';
update public.brand_events set community_id = brand_slug where brand_slug is not null and community_id = 'raelynn';
create index if not exists brand_events_community_idx
  on public.brand_events (community_id, starts_at);

-- campaigns
alter table public.campaigns add column if not exists community_id text not null default 'raelynn';
update public.campaigns set community_id = brand_slug where brand_slug is not null and community_id = 'raelynn';

-- member_actions (CTAs)
alter table public.member_actions add column if not exists community_id text not null default 'raelynn';
update public.member_actions set community_id = brand_slug where brand_slug is not null and community_id = 'raelynn';

-- member_action_completions: no brand_slug today. Inherit from action row.
alter table public.member_action_completions add column if not exists community_id text not null default 'raelynn';
update public.member_action_completions c
...
-- offers: no per-brand scoping today — future launches will have per-community catalogs.
...
create index if not exists points_ledger_community_member_idx
  on public.points_ledger (community_id, member_id, created_at desc);

-- notifications: already indexed by member_id; add community for future filtering.
alter table public.notifications add column if not exists community_id text not null default 'raelynn';

-- member_badges: a member can earn the same badge in multiple communities.
-- The current PK (member_id, badge_slug) blocks multi-community earns — we
-- widen it to (member_id, badge_slug, community_id) so e.g. "Welcome" can
-- be earned once per community.
alter table public.member_badges add column if not exists community_id text not null default 'raelynn';
-- Only drop-and-recreate the PK if it's the old 2-column shape. Idempotent guard:
do $$
begin
  if exists (
    select 1 from information_schema.table_constraints
    where table_schema = 'public' and table_name = 'member_badges'
      and constraint_type = 'PRIMARY KEY'
      and constraint_name = 'member_badges_pkey'
  ) and not exists (
    ...
    alter table public.member_badges drop constraint member_badges_pkey;
    alter table public.member_badges add primary key (member_id, badge_slug, community_id);
  end if;
end $$;
...
-- member_brand_following is supplanted by member_community_memberships in 4b/4c.
...
-- When a member's street-team membership becomes active, mirror memberships
-- into every active brand community. Deliberately excludes `brand` types
...
begin
  if new.community_id = 'street-team' and new.status = 'active'
    ...
    loop
      insert into member_community_memberships (member_id, community_id, joined_at, status)
      values (new.member_id, r.slug, now(), 'active')
      on conflict (member_id, community_id) do nothing;
    end loop;
  end if;
  return new;
...
drop trigger if exists member_community_memberships_mirror_street_team
  on public.member_community_memberships;
create trigger member_community_memberships_mirror_street_team
...
-- this migration. Current policies (auth.uid() = member_id etc.) remain in
...
alter table public.member_community_memberships enable row level security;
...
-- member_community_memberships: members read their own rows; community admins
-- can read their community's member roster.
drop policy if exists memberships_own_read on public.member_community_memberships;
create policy memberships_own_read on public.member_community_memberships
  for select using (
    auth.uid() = member_id
    or is_admin_of(community_id)
  );

-- Members can update their own membership (e.g. leave community, change status).
...
--   from member_community_memberships group by 1;
-- -- Expected: one row for 'raelynn' matching count(*) on members.
--
-- -- Every scoped table has a community_id?
-- select 'community_posts' as tbl, community_id, count(*) from community_posts group by 2
-- union all select 'brand_events',       community_id, count(*) from brand_events group by 2
-- union all select 'campaigns',           community_id, count(*) from campaigns group by 2
-- union all select 'member_actions',         community_id, count(*) from member_actions group by 2
-- union all select 'points_ledger',       community_id, count(*) from points_ledger group by 2
-- union all select 'member_badges',          community_id, count(*) from member_badges group by 2
