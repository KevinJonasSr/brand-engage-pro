...
--
-- Phase 5d (migration 0015) added public.points_multiplier(member_id, community_id)
-- but no trigger consumed it. This migration updates the four community-scoped
-- point-award functions so premium members earn 1.5× on every engagement:
--
...
--
-- community_id = brand_slug. community_posts has it directly; the other three
-- tables reference a post_id and join to community_posts to find the slug.
--
-- Note on source_ref guard: points are idempotent per source_ref, so a member who
...
--   - award_event_rsvp_points     (event_rsvps → brand_events.brand_slug)
--   - award_member_action_points     (member_action_completions → member_actions.brand_slug,
...
begin
  ...
  if not exists (select 1 from points_ledger where source_ref = ref_id) then
    insert into points_ledger (member_id, delta, source, source_ref, note)
    values (
      new.member_id, award, 'challenge', ref_id,
...
-- Free member should see 1.0 multiplier, premium member should see 1.5.
-- select public.points_multiplier(
--   (select member_id from member_community_memberships
...
-- );  -- expect 1.5
--
-- Inspect the most recent ledger entries to confirm premium members show
