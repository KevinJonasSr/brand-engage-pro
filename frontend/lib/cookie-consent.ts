 ...
 *
 * Soft-launch UX: Accept-only banner (parity with Fan Engage least-confused path).
 * Referral cookies may only be set after an explicit Accept.
 */

export const COOKIE_CONSENT_STORAGE_KEY = "memberengage_cookie_consent";

export type CookieConsentChoice = "accept";
...
function parseRecord(raw: string | null): CookieConsentRecord | null {
  ...
  try {
    ...
    // Legacy "decline" records are treated as no consent (block non-essential).
    if (parsed.choice !== "accept") return null;
    return {
      choice: "accept",
 ...
 * an explicit Accept. No choice yet blocks them.
 */
